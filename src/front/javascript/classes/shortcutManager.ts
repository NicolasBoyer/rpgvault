...
export class ShortcutManager {
    ...
    private static keyDown(pEvent: KeyboardEvent): void {
        if (Object.keys(ShortcutManager.shortCuts).some((pKey): boolean => pKey.split(',').includes(pEvent.key)) && States.editMode) {
            ...
            const shortcut = ShortcutManager.shortCuts[Object.keys(ShortcutManager.shortCuts).find((pKey): boolean => pKey.split(',').sort().join() === Object.keys(ShortcutManager.keysPress).filter((pKey): boolean => ShortcutManager.keysPress[pKey]).map((pKey): string => pKey).sort().join()) as string]
            if (!shortcut) return
